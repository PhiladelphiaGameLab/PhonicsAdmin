$(function () {
  ...
  var stopRecorder = function(recorder) {
    ...
    recorder.exportWAV(function(wav) {
      var url = window.URL.createObjectURL(wav) || window.webkitURL.createObjectURL(wav);
      $("a#export").attr("href", url);
      console.log("Getting the upload area");
      document.getElementById("upload").onclick = (function(blob){
		// upload to server.
		console.log("Calling the upload function");
		AjaxUpload({files:[blob]}, "testingAudioUpload.wav");
      })(wav)
